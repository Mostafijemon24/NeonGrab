...
async function resolveSingleUrl(url: string): Promise<ResolvedItem | null> {
  ...
  if (probed.items?.length === 1) return probed.items[0];
  if (probed.items && probed.items.length > 1) return probed.items[0];

  if (Capacitor.getPlatform() === "android") {
    const probedLoose = await probeOne(url, true);
    if (probedLoose.items?.length) return probedLoose.items[0];
  }
