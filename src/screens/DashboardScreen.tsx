...
export function DashboardScreen() {
  ...
  const handleDownload = async () => {
    ...
    setError(null);
    setBatchHint(null);
    const { ok, count, started, error: errKey, errorDetail } = await enqueueUrl(
      url,
      mode === "batch",
    );
    setLoading(false);
    if (!ok) {
      ...
      const key = known.includes(errKey as (typeof known)[number])
        ...
        : "invalidUrl";
      /* Show the real yt-dlp/native error when available, else fall back to i18n */
      const isRawCode = (s?: string) =>
        !s || /^[A-Z][A-Z0-9_]*$/.test(s.trim()) || s.trim().length === 0;
      const detail = isRawCode(errorDetail) ? undefined : errorDetail;
      setError(detail ?? tr(key));
      return;
    }
    if (started === 0) {
