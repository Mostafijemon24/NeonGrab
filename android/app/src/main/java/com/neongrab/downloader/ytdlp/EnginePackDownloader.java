...
final class EnginePackDownloader {
    ...
    static void reconcileEngineForDevice(Context app) {
        ...
        if (EngineNativeLoader.verifyBinDir(persistent)
    ...
    private static void repairMarker(Context app) {
        try {
    ...
    private static void migrateLegacyInstall(Context app) {
        ...
        File legacyCodeCache = new File(app.getCodeCacheDir(), "engine/bin");
        if (EngineNativeLoader.verifyBinDir(legacyCodeCache)) {
            try {
                if (!persistent.exists()) persistent.mkdirs();
                EngineNativeLoader.syncBinDir(legacyCodeCache, persistent);
                ...
                EngineNativeLoader.syncBinDir(persistent, EngineNativeLoader.execBinDir(app));
            } catch (Exception ignored) {
                /* fall through */
            }
            if (EngineNativeLoader.verifyBinDir(persistent)) return;
    ...
    static void ensureDownloaded(Context context, Progress progress) throws Exception {
        ...
        if (!EngineAbi.validateEngineBinDir(binDir)) {
            deleteRecursive(binDir);
            throw new Exception(EngineAbi.wrongPackMessage());
        }
    ...
    private static boolean installFromAssets(Context app, File dest, Progress progress)
            throws Exception {
        String[] assetNames =
                EngineAbi.deviceUsesArm64()
                        ? new String[] {"engine-pack-arm64.zip"}
                        : new String[] {"engine-pack-x86_64.zip", "engine-pack.zip"};
        for (String assetName : assetNames) {
            if (copyAssetToFile(app, assetName, dest, progress)) {
    ...
    private static boolean copyAssetToFile(
            Context app, String assetName, File dest, Progress progress) {
