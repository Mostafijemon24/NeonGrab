...
final class YtDlpProcessRunner {
    ...
    static YoutubeDLResponse execute(
        ...
        if (exitCode != 0 && (out == null || out.trim().isEmpty())) {
            ...
            /* Python "I/O operation on closed file." is a harmless atexit flush failure —
               it can occur even when the download completed. Treat as non-fatal so the
               caller can check whether a media file was actually saved. */
            if (errMsg.contains("I/O operation on closed file")) {
    ...
    private static void applyEnv(ProcessBuilder pb, Object ytdlp, File binDir) throws Exception {
        ...
        /* Force unbuffered stdout so Python never has pending bytes to flush at exit.
           Without this, Python may raise "I/O operation on closed file." when the JVM
           closes the read-end of the pipe before Python flushes its internal buffer. */
